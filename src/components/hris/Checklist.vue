              ...
              <v-toolbar dense style="">
                ...
                <template v-if="$vuetify.breakpoint.mdAndUp">
                  ...
                  dark
                    ...
                    elevation="2"
                    @click="NewChecklist()"
                    v-if="$store.getters.valAccess(212)"
                  >
              ...
              <template v-slot:item.action="props">
                <v-btn
                  elevation="2"
                      ...
                      small
                      color="orange"
                      v-if="$store.getters.valAccess(213)"
                      @click="UpdateAllChecklisttype(props.item.id, props.item)"
                  >
                  <v-icon
                          small
                          @click="UpdateAllChecklisttype(props.item.id, props.item)"
                  ...
                  elevation="2"
                      ...
                      small
                      color="red"
                      v-if="$store.getters.valAccess(214)"
    ...
    >
      <v-card>
          ...
          >
              ...
              <v-toolbar-title>Add Requirements Checklist</v-toolbar-title>
              <v-spacer></v-spacer>
              <v-toolbar-items>
              </v-toolbar-items>
          </v-toolbar>
          ...
          <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
              <h4>Note: Here you can create Requirements Checklist</h4>
          </v-col>
    ...
    <v-dialog
      ...
      <v-card>
          ...
          >
              ...
              <v-toolbar-title>Edit Requirements Checklist</v-toolbar-title>
              <v-spacer></v-spacer>
              <v-toolbar-items>
              </v-toolbar-items>
          </v-toolbar>
          ...
          <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
              <h4>Note: Here you can edit Requirements Checklist</h4>
          </v-col>
...
<script>
  ...
  import checklistservices from '../services/checklistservices';
    ...
    export default {
      ...
      mounted(){
          this.$store.state.title = "Requirements Checklist";
      },
      computed: {
      ...
      methods: {
        FetchAllChecklist(){ // fetch all onboarding type
            let payload = {
                parent_id: this.$store.getters.getParentID
            };
            checklistservices.FetchAllChecklist(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.checklistdata = response.data.data;
                }
            }).catch(error => console.log(error));
        },
        UpdateChecklist(id, name, code, description, created_by){ // fetch info for update onboarding type
            this.forUpdate = {
                id: id,
                name: name,
                code: code,
        ...
        UpdateAllChecklisttype(id, item){ // fetch info for update onboarding type
            console.log(id, item);
            this.checklist.name = item.name
            this.checklist.type = item.type
            this.forAllUpdate = {
                id: id,
        ...
        confirmUpdate(){ // edit on boarding type
            ...
            console.log(payload)
            checklistservices.UpdateChecklist(payload, this.$store.getters.getAccessToken).then(response => {
                    if(response.data.success){
                        this.$store.state.snackbar.message =  this.checklist.type + ' has been updated.';
        ...
        SaveChecklist(){ // save on boarding type
            ...
            if(val){
                ...
                console.log(payload);
                checklistservices.SaveChecklist(payload, this.$store.getters.getAccessToken).then(response => {
                    if(response.data.success){
                        this.fullscreendialog = false;
                        this.$store.state.snackbar.message = this.checklist.type + ' has been created.';
                        this.$store.state.snackbar.show = true;
                        this.FetchAllChecklist();
                        this.ClearChecklist();
                        this.$refs.form.resetValidation();
        ...
        confirmDelete(){ // delete on boarding type
            let payload = {
                id: this.forDelete['id']
            }
            console.log(payload);
            checklistservices.DeleteChecklist(payload, this.$store.getters.getAccessToken).catch(error =>{
                this.errorDAW = error.response.data.message
                this.$store.state.snackbar.message = error.response.data.message;
                this.$store.state.snackbar.show = true;
                    this.$store.state.snackbar.colors = 'xred';
            ...
            this.$store.state.snackbar.message = 'Requirements Checklist has been deleted.';
            this.$store.state.snackbar.show = true;
