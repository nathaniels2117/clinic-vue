<template>
    <v-container style="max-width: 95%; position: relative;">
                                    ...
                                    <template v-slot:item.action="props">
                                        <v-row>
                                            ...
                                            elevation="2"
                                                ...
                                                outlined
                                                color="orange"
                                                v-if="$store.getters.valAccess(238)"
                                                @click="UpdateEmployee(props.item, 'approved')"
                                            >
                                                <v-icon small>
                                            ...
                                            elevation="2"
                                                ...
                                                outlined
                                                color="red"
                                                v-if="$store.getters.valAccess(239)"
                                                @click="UpdateEmployee(props.item, 'disapproved')"
                                            >
                                                <v-icon small>
                                                    mdi-file-edit-outline
                                                </v-icon>
        ...
        <!-- Dialog  for set up  Division, Role, Branch -->  
        <template>
    ...
    export default {
      ...
      data: () => ({
        ...
        headersMap: [
            ...
            { text: 'Action', value: 'action' ,width: '320px',sortable: false}
        ],
      ...
      mounted(){
            this.$store.state.title = "Registration Approval Form"
      ...
      methods: {
            FetchAllUserForApproval(){ // fetch all user for approval in register
                let payload = {
                    parent_id: this.$store.getters.getParentID
                };
                userservices.FetchAllUserForApproval(payload, this.$store.getters.getAccessToken).then(response => {
            ...
            UpdateEmployee(item, action){
                this.forUpdate = {
                    id: item.id,
                    action: action
                }
                if(action == 'approved'){
                    this.listempdialoglabel = 'Approve';
            ...
            SaveorUpdate(){
                ...
                if(val){
                    if(this.valid == true){
                        this.ApproveDisapproveEmployee()
                    }
                }
            },
            validationfalse(){
            ...
            ApproveDisapproveEmployee(){ // approval or disapprove of employee
                let loader = this.$loading.show();
                if(this.forUpdate.action == 'approved'){
                    this.user_information.flag = 0;
                }else{
                    this.user_information.flag = 2
                }
                ...
                let payload = {
                    ...
                    parent_id : this.$store.getters.getParentID,
                    type : this.forUpdate.action
                };
                userservices.ApproveDisapproveEmployee(payload, this.$store.getters.getAccessToken).then(response => {
                    if(response.data.success){
                        loader.hide();
            ...
            DisapproveEmployee(){ // disapprove of employee
                let loader = this.$loading.show();
            ...
            UpdateSchedule(other_information_id, user_id,schedule_id){
            ...
            confirmUpdate(){ // update user other information schedule
            let payload = {
