    ...
    >
      ...
      <template v-if="$vuetify.breakpoint.mdAndUp">
        ...
        dark
          color="orange"
          elevation="2"
          @click="ExportCategoryInformation()"
        >
        ...
        <template v-if="$vuetify.breakpoint.smAndDown">
            ...
            <v-menu offset-y>
              ...
              <v-list>
                ...
                <div v-else>
                  <v-list-item-title @click="ExportCategoryInformation()"> <v-icon>{{ item.icon }}</v-icon>{{ item.text }}</v-list-item-title>
                </div>
                </v-list-item>
              </v-list>
            </v-menu>
                ...
                <v-data-table
                    ...
                    <template v-slot:item.name="props">
                        <v-edit-dialog
                            ...
                            @save="UpdateCategoryInformation(props.item.id, props.item.name)"
                        >
                        {{ props.item.name }}
                        <template v-slot:input
                        v-if="$store.getters.valAccess(31) || $store.getters.valAccess(72)"
                        >
                            <v-text-field
                                color="orange"
                    ...
                    <template v-slot:item.action="props">
                      ...
                      elevation="2"
                          ...
                          v-if="$store.getters.valAccess(43)"
                            @click="EditCategoryInformation(props.item.id, props.item)"
                      >
                      <v-icon
                            small
                        >
                      ...
                      elevation="2"
                          ...
                          small
                          color="red"
                          v-if="$store.getters.valAccess(32) || $store.getters.valAccess(73)"
                            @click="RemoveCategoryInformation(props.item.id, props.item)"
                      >
                      <v-icon
                            small
                            @click="RemoveCategoryInformation(props.item.id, props.item)"
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="UpdateAllCategoryInformation()"
            >
            Confirm
          </v-btn>
        </center>
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="ImportCategoryInformation()"
            >
            Confirm
          </v-btn>
        </center>
    ...
    >
      <v-card>
        ...
        <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can add another category information.</h4>
        </v-col>
  ...
  >
    ...
    >
      <v-card>
        ...
        <v-col sm="12" md="6" offset-md="3"> 
            <v-text-field
                v-model="update_category_information.name"
                label="Category"
                required
                outlined
                 :counter="50"
...
<script>
  ...
  export default {
    ...
    mounted(){
        this.$store.state.title = "Category";
    },
    computed: {
    ...
    methods: {
        ...
        EditCategoryInformation(id, data){
          this.updatedialog = true;
          this.update_category_information = {
              name: data.name
              };
          this.updateAllID = id;
        },
      uploadFile() {
        ...
        UpdateAllCategoryInformation(){

        let payload = {
          id: this.updateAllID,
          category_information: JSON.stringify(this.update_category_information)
        };
        categoryservices.updateCategoryInformation(payload, this.$store.getters.getAccessToken).then(response => {
          if(response.data.success){
        ...
        ClearSaveCategory(){
            this.category_information.name = '';
        },
        confirmUpdate(){
            let payload = {
                id: this.foUpdate['id'],
                category_information: JSON.stringify({
                    name: this.foUpdate['name']
                })
            }
            categoryservices.updateCategoryInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Category information has been updated.';
            this.$store.state.snackbar.show = true;
            this.$store.state.snackbar.colors = 'xgreen';
            this.$store.state.snackbar.icon = "mdi-check-circle-outline";            
            this.FetchCategoryInformation();
        ...
        UpdateCategoryInformation(id, name){
        ...
        confirmDelete(){
            this.category.splice(this.category.indexOf(this.foDelete['data']), 1);

            let payload = {
                id: this.foDelete['id']
            };

            categoryservices.removeCategoryInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));

            this.$store.state.snackbar.message = 'Category information has been removed.';
            this.$store.state.snackbar.show = true;
        ...
        RemoveCategoryInformation(id, data){
            this.foDelete = {
                id: id,
                data: data
            }
        ...
        // export xlsx file
        ExportCategoryInformation(){

            let payload = {
                user_id: this.$store.getters.getParentID
            };

            categoryservices.exportCategoryInformation(payload, this.$store.getters.getAccessToken)
            .then(response => {
        ...
        // end export xlsx file


        // download sample xlsx file template
        DownloadTemplate(){
          ...
          categoryservices.downloadTemplate(payload, this.$store.getters.getAccessToken)
          .then(response => {
              const url = window.URL.createObjectURL(new Blob([response.data]))
              const link = document.createElement('a')
              link.href = url
              link.setAttribute('download','Category_format.xlsx')
              document.body.appendChild(link)
              link.click()
          }).catch(error => console.log(error));

          },
        // end download sample xlsx file template

        // import xlsx file
          ImportCategoryInformation() {
            ...
            categoryservices.importCategoryInformation(formData,headers, this.$store.getters.getAccessToken)
            .then(response => {

                    loader.hide();

            this.$store.state.snackbar.message = 'Category information has been imported.';
            this.$store.state.snackbar.show = true;
            this.FetchCategoryInformation();
