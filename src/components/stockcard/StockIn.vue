                        ...
                        <div class="row">
                            <div class="col-sm-4">
                                <v-combobox
                                    ...
                                    label="Supplier"
                                    :items="supplier_information"
                                    :rules="[v => !!v || 'Supplier Information is required']"
                                    color="orange"
                                ></v-combobox>
                            </div>
                            <div class="col-sm-4">
...
<script>
  ...
  export default {
    ...
    mounted(){
        this.$store.state.title = "Stock In Information";
        this.stock_action = this.stock_actions[0]
    },
    methods: {
        ...
        // change location from depending on location to
        changelocfrom(){
            this.branch_location_from = this.branch_location_to['text']
            this.fetchProductInformation()
        },

        // auto populate fields (uom, approved cost, stocks) onclick product
       ...
       FetchBranchInformation(){
        ...
        FetchSupplierInformation(){
            ...
            supplierservices.fetchSupplierInformation(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    response.data.result.forEach((value) => {
                        this.supplier_information.push({
                            text: value.name,
                            value: value.id
                        });
                    });
