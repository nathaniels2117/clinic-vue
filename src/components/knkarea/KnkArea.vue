                      ...
                      <v-btn
                        ...
                        <v-btn
                          ...
                          v-if="$store.getters.valAccess(16)"
                            @click="RemoveKnkAreaInformation(props.item.id, props.item)"
                        >
                        <v-icon
                                small
                            @click="RemoveKnkAreaInformation(props.item.id, props.item)"
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="ImportKnkAreaInformation()"
            >
              Confirm
          </v-btn>
        </center>
    ...
    >
      <v-card>
        ...
        <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can add another area information.</h4>
        </v-col>
        <v-col sm="12" md="6" offset-md="3"> 
            <v-text-field
                color="orange"
                v-model="knkarea_information.name"
                label="Area"
                required
                outlined
                 :counter="50"
    ...
    >
        <v-card>
            ...
            >
                ...
                <v-toolbar-title>Edit Area</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items>
                </v-toolbar-items>
            </v-toolbar>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        SaveKnkAreaInformations(){
            let payload = {
                knkarea_information: JSON.stringify(this.knkarea_information)
                };
            knkareaservices.saveKnkAreaInformations(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.dialog = false;
                    this.FetchKnkAreaInformation();
        ...
        ClearSaveDivision(){
            this.knkarea_information.name = '';
        },
        confirmUpdate(){
            let payload = {
                id: this.forAllUpdate['id'],
                knkarea_information: JSON.stringify({
                    name: this.knkarea_information.name
                })
            }
            knkareaservices.updateKnkAreaInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
        ...
        UpdateKnkAreaInformation(id, name){
            this.foUpdate = {
                id: id,
                name: name
            }
        ...
        confirmDelete(){
            ...
            knkareaservices.removeKnkAreaInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Area information has been removed.';
            this.$store.state.snackbar.show = true;
            this.confirmation = false
            this.$store.state.snackbar.colors = 'xgreen';
                    this.$store.state.snackbar.icon = "mdi-check-circle-outline";   
        },
        RemoveKnkAreaInformation(id, data){
            this.foDelete = {
                id: id,
                data: data
            }
          ...
          UpdateAllDivision(id, item){
            console.log(id, item)
            this.knkarea_information.name = item.name
            this.forAllUpdate = {
                id: id,
            }
