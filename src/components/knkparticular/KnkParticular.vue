                      ...
                      <v-btn
                        ...
                        <v-btn
                          ...
                          v-if="$store.getters.valAccess(16)"
                            @click="RemoveKnkParticularInformation(props.item.id, props.item)"
                        >
                        <v-icon
                                small
                            @click="RemoveKnkParticularInformation(props.item.id, props.item)"
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="SaveKnkParticularInformations()"
            >
            Confirm
          </v-btn>
        </center>
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="ImportKnkParticularInformation()"
            >
              Confirm
          </v-btn>
        </center>
    ...
    >
      <v-card>
        ...
        >
          ...
          <v-toolbar-title>Add Particular</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
          </v-toolbar-items>
        </v-toolbar>
        ...
        <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can add another particular information.</h4>
        </v-col>
        <v-col sm="12" md="6" offset-md="3"> 
            <v-text-field
                color="orange"
                v-model="knkparticular_information.name"
                label="Particular"
                required
                outlined
                 :counter="50"
    ...
    >
        <v-card>
            ...
            >
                ...
                <v-toolbar-title>Edit Particular</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items>
                </v-toolbar-items>
            </v-toolbar>
            <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can edit particular information.</h4>
        </v-col>
        <v-col sm="12" md="6" offset-md="3">
            <v-text-field
                color="orange"
...
<script>
  ...
  export default {
    ...
    mounted(){
        this.$store.state.title = "Particular";
    },
    computed: {
      showHeaders () {
    ...
    data: () => ({
      ...
      page: 1,
        ...
        valid: true,
        nameRules: [
        v => !!v || 'Particular name is required',
        ],
        confirmation: false,
    ...
    methods: {
        ...
        SaveKnkParticularInformations(){
            let payload = {
                knkparticular_information: JSON.stringify(this.knkparticular_information)
                };
            knkparticularservices.saveKnkParticularInformations(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.dialog = false;
                    this.FetchKnkParticularInformation();
        ...
        ClearSaveDivision(){
            this.knkparticular_information.name = '';
        },
        confirmUpdate(){
            let payload = {
                id: this.forAllUpdate['id'],
                knkparticular_information: JSON.stringify({
                    name: this.knkparticular_information.name
                })
            }
            knkparticularservices.updateKnkParticularInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Particular information has been updated.';
            this.$store.state.snackbar.show = true;
            this.FetchKnkParticularInformation();
        ...
        UpdateKnkParticularInformation(id, name){
        ...
        confirmDelete(){
            ...
            knkparticularservices.removeKnkParticularInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));

            this.$store.state.snackbar.message = 'Particular information has been removed.';
            this.$store.state.snackbar.show = true;
            this.confirmation = false
            this.$store.state.snackbar.colors = 'xgreen';
                    this.$store.state.snackbar.icon = "mdi-check-circle-outline";   
        },
        RemoveKnkParticularInformation(id, data){
            this.foDelete = {
                id: id,
                data: data
            }
          ...
          UpdateAllDivision(id, item){
            console.log(id, item)
            this.knkparticular_information.name = item.name
            this.forAllUpdate = {
                id: id,
            }
