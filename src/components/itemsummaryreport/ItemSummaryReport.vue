...
<template v-if="$vuetify.breakpoint.mdAndUp">
  ...
  <v-btn
    ...
    color="orange"
    elevation="2"
    @click="ExportItemSummaryInformation()"
    v-if="$store.getters.valAccess(267)"
  >
...
<br>
              <v-data-table
                      ...
                      'disable-items-per-page': true,
                          ...
                          >
                        <template v-slot:item.total_net_sales="props">
                            {{ parseFloat(props.item.total_net_sales).toFixed(2) }}
                        </template>
                        <template v-slot:item.total_qty_sold="props">
                            {{ parseFloat(props.item.total_qty_sold).toFixed(2) }}
                        </template>
                        ...
                        <template v-slot:item.price="props">
                          ...
                          outlined
                          color="orange"
                          @click="gotoItemSummaryReportPreview(props.item.id)"
                        >
                        <v-icon
                                    small
                                    @click="gotoItemSummaryReportPreview(props.item.id)"
...
import itemsummaryreportservices from "../services/itemsummaryreportservices";
  import moment from 'moment';
    ...
    export default {
      ...
      data: () => ({
        ...
        selectedHeaders: [],
        headersMap: [
            { text: 'Product Name', value: 'product_info' },
            { text: 'Price', value: 'price' },
            { text: 'Total Quantity', value: 'total_qty_sold' },
            { text: 'Total Sales', value: 'total_net_sales' }
          ],
      ...
      mounted(){
          this.$store.state.title = "Top Selling Product";
      },
      watch: {
            menu_year(val) {
      ...
      methods: {
        ...
        checkdaily(){
          ...
          gotoItemSummaryReportPreview(id) {
              localStorage.setItem('transactionID',id)
              window.open("/previewitemsummaryreport","_blank")
          },
          gotoItemSummaryPrint(id) {
              localStorage.setItem('transactionID',id)
              window.open("/printitemsummaryreport","_blank")
          },
          // fetch date to get data
          getDates(){
            ...
            let payload = {
                      itemsummary_report: JSON.stringify(filters),
                      business_branch_id: this.$store.getters.getBranchID
                  }
              itemsummaryreportservices.fetchItemSummaryReportInformation(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.itemsummaryreport = response.data.result;
                    this.itemsummaryreportlength = this.itemsummaryreport.length;
                    for (var z = 0 ; z < this.itemsummaryreportlength ; z++){
                    this.total_quantity += parseFloat(this.itemsummaryreport[z].total_qty_sold);
                    this.total_amount += parseFloat(this.itemsummaryreport[z].total_net_sales)
                    }                       
                  }
              }).catch(error => console.log(error));
        },  
        ExportItemSummaryInformation(){
          var filters = {
                date_from: this.date_from,
                date_to: this.date_to
            }
          ...
          itemsummaryreportservices.exportItemSummaryInformation(payload, this.$store.getters.getAccessToken)
          .then(response => {
              const url = window.URL.createObjectURL(new Blob([response.data]))
              const link = document.createElement('a')
              link.href = url
              link.setAttribute('download','ItemSummary.xlsx')
