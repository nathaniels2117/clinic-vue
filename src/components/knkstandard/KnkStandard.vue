                      ...
                      <v-btn
                        ...
                        <v-btn
                          ...
                          v-if="$store.getters.valAccess(16)"
                            @click="RemoveKnkStandardInformation(props.item.id, props.item)"
                        >
                        <v-icon
                                small
                            @click="RemoveKnkStandardInformation(props.item.id, props.item)"
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="SaveKnkStandardInformations()"
            >
            Confirm
          </v-btn>
        </center>
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="ImportKnkStandardInformation()"
            >
              Confirm
          </v-btn>
        </center>
    ...
    >
      <v-card>
        ...
        >
          ...
          <v-toolbar-title>Add Standard</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-toolbar-items>
          </v-toolbar-items>
        </v-toolbar>
        <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can add another standard information.</h4>
        </v-col>
        <v-col sm="12" md="6" offset-md="3"> 
            <v-text-field
                color="orange"
                v-model="knkstandard_information.name"
                label="Standard"
                required
                outlined
                 :counter="50"
            ...
            ></v-text-field>
                <v-combobox
                v-model="knkstandard_information.particular_id"
                label="Particular"
                required
                outlined
                :items="particular"
    ...
    >
        <v-card>
            ...
            >
                ...
                <v-toolbar-title>Edit Standard</v-toolbar-title>
                <v-spacer></v-spacer>
                <v-toolbar-items>
                </v-toolbar-items>
            </v-toolbar>
            <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can edit standard information.</h4>
        </v-col>
        <v-col sm="12" md="6" offset-md="3">
            <v-text-field
                color="orange"
...
<script>
  ...
  export default {
    name: 'KnkStandard',
    created() {
        this.FetchKnkStandardInformation();
    ...
    mounted(){
        this.$store.state.title = "Standard";
    },
    computed: {
      showHeaders () {
    ...
    data: () => ({
      ...
      page: 1,
        ...
        headersMap: [
          ...
          { text: 'Particular', sortable: false, value: 'particular' },
    ...
    methods: {
        ...
        SaveKnkStandardInformations(){
          var myBranchCode = this.knkstandard_information['particular_id']
            this.knkstandard_information['particular_id'] = myBranchCode['value']
            let payload = {
                knkstandard_information: JSON.stringify(this.knkstandard_information)
                };
            knkstandardservices.saveKnkStandardInformations(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.dialog = false;
                    this.FetchKnkStandardInformation();
        ...
        ClearSaveDivision(){
            this.knkstandard_information.name = '';
        },
        confirmUpdate(){
          this.FetchKnkParticularInformation();
            let payload = {
                id: this.forAllUpdate['id'],
                knkstandard_information: JSON.stringify({
                    name: this.knkstandard_information.name
                })
            }
            knkstandardservices.updateKnkStandardInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Standard information has been updated.';
            this.$store.state.snackbar.show = true;
            this.FetchKnkStandardInformation();
        ...
        UpdateKnkStandardInformation(id, name){
            this.foUpdate = {
                id: id,
                name: name
            }
        ...
        confirmDelete(){
            ...
            knkstandardservices.removeKnkStandardInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Standard information has been removed.';
            this.$store.state.snackbar.show = true;
            this.confirmation = false
            this.$store.state.snackbar.colors = 'xgreen';
            this.$store.state.snackbar.icon = "mdi-check-circle-outline";   
        },
        RemoveKnkStandardInformation(id, data){
            this.foDelete = {
                id: id,
                data: data
            }
          ...
          UpdateAllDivision(id, item){
            console.log(id, item)
            this.knkstandard_information.name = item.name
            this.forAllUpdate = {
                id: id,
            }
