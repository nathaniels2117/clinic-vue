<template>
  <v-container>
          ...
          <v-card-text>
            ...
            <template>
              <v-toolbar dense style="">
                <v-row>
                  <v-col cols="12">
                    <v-text-field v-model="search" append-icon="mdi-magnify" placeholder="Search" hide-details
                      color="orange" overflow solo dense style="min-width: 60px;"></v-text-field>
                  </v-col>
                </v-row>
                &nbsp;
                ...
                <template v-if="$vuetify.breakpoint.mdAndUp">
                  <v-btn medium dark color="orange" elevation="2" @click="NewDivision()"
                ...
                <template v-if="$vuetify.breakpoint.smAndDown">
                  <v-btn medium dark color="orange" elevation="2" style="padding-left: 12px;padding-right: 12px;"
                    @click="NewDivision()" v-if="$store.getters.valAccess(14)">
            ...
            <br>
            <!-- Division data table -->
            <v-data-table :headers="showHeaders" :items="division" :search="search" :page.sync="page"
              ...
              <template v-slot:item.createdby="props">
                {{ props.item.created_info.personal_information.lastname }}, {{
                  props.item.created_info.personal_information.firstname }}
              </template>

              <template v-slot:item.division_head_id="props">
                {{ props.item.department_head_info != null ?
                  props.item.department_head_info.personal_information.lastname + ' , ' +
                  props.item.department_head_info.personal_information.firstname : '' }}
              </template>

              <template v-slot:item.immediate_superior_id="props">
                {{ props.item.immediate_superior_info != null ?
                  props.item.immediate_superior_info.personal_information.lastname + ' , ' +
                  props.item.immediate_superior_info.personal_information.firstname : '' }}
              </template>
              ...
              <template v-slot:item.action="props">
                <v-btn elevation="2" class="mx-2" dark outlined small color="orange" v-if="$store.getters.valAccess(15)"
                  @click="UpdateAllDivision(props.item.id, props.item)">
                  <v-icon small @click="UpdateAllDivision(props.item.id, props.item)">
                    mdi-file-edit-outline
                  </v-icon>
                  Edit
                </v-btn>
                <v-btn elevation="2" class="mx-2" dark small outlined color="red" v-if="$store.getters.valAccess(16)"
                  @click="RemoveDivisionInformation(props.item.id, props.item)">
                  <v-icon small @click="RemoveDivisionInformation(props.item.id, props.item)">
                    mdi-delete
                  </v-icon>
                  Delete
                </v-btn>
        ...
        <v-dialog v-model="saveconfirmation" persistent max-width="290">
          <v-card>
            ...
            <center>
              ...
              <v-btn elevation="2" class="mx-2" dark outlined small color="orange" @click="SaveDivisionInformations()">
                Confirm
              </v-btn>
            </center>
            <br>
        ...
        <v-dialog v-model="editconfirmation" persistent max-width="290">
          <v-card>
            ...
            <center>
              ...
              <v-btn elevation="2" class="mx-2" dark outlined small color="orange" @click="confirmUpdate()">
                Confirm
              </v-btn>
            </center>
            <br>
      ...
      <v-row justify="center">
        <v-dialog v-model="confirmation" persistent max-width="290">
          <v-card>
            ...
            <center>
              <v-btn elevation="2" class="mx-2" dark outlined small color="red" @click="changeStatusDialog(false)">
                Cancel
              </v-btn>
              <v-btn elevation="2" class="mx-2" dark outlined small color="orange" @click="confirmDelete()">
                Confirm
              </v-btn>
            </center>
            <br>
    ...
    <template>
      <v-row justify="center">
        <v-dialog v-model="uploadconfirmation" persistent max-width="290">
          <v-card>
            <v-card-title class="text-h5">
              Upload
            </v-card-title>
            ...
            <center>
              ...
              <v-btn elevation="2" class="mx-2" dark outlined small color="orange" @click="ImportDivisionInformation()">
                Confirm
              </v-btn>
            </center>
            <br>
      ...
      <v-dialog v-model="dialog" fullscreen hide-overlay transition="dialog-bottom-transition">
        <v-card>
          ...
          <v-col sm="12" md="6" offset-md="3">
            <v-text-field color="orange" v-model="division_information.name" label="Division" required outlined
              :counter="50" :rules="nameRules"></v-text-field>
            <center>
              <v-col sm="12" md="12">
                <v-btn color="orange" block outlined @click="validate()" id="btn-next"
                  class="btn btn-success btn-sm p-2" style="font-size: 12px;">
                  Save
                </v-btn>
              </v-col>
            </center>
    ...
    <v-dialog fullscreen v-model="editfullscreendialog" hide-overlay transition="dialog-bottom-transition">
      <v-card>
        <v-form ref="form" v-model="valid" lazy-validation>
          ...
          <v-col sm="12" md="6" offset-md="3">
            <v-combobox outlined color="orange" v-model="user_info" label="Division Head"
              :clearable="this.user_info != ''" :items="listemployeedata"></v-combobox>
          </v-col>
          <v-col sm="12" md="6" offset-md="3">
            <v-combobox outlined color="orange" v-model="immediate_superior_info"
              label="Immediate Superior (Supervisor)" :clearable="this.immediate_superior_info != ''"
              :items="listemployeedata"></v-combobox>
          </v-col>
          <center>
            <v-col sm="12" md="6">
              <v-btn color="orange" block outlined
