                      ...
                      <v-btn
                        ...
                        <v-btn
                          ...
                          v-if="$store.getters.valAccess(16)"
                            @click="RemoveKnkRoomtypeInformation(props.item.id, props.item)"
                        >
                        <v-icon
                                small
                            @click="RemoveKnkRoomtypeInformation(props.item.id, props.item)"
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="SaveKnkRoomtypeInformations()"
            >
            Confirm
          </v-btn>
        </center>
      ...
      <v-card>
        ...
        <center>
          ...
          <v-btn
            ...
            small
            color="orange"
            @click="ImportKnkRoomtypeInformation()"
            >
              Confirm
          </v-btn>
        </center>
    ...
    >
      <v-card>
        ...
        <v-col sm="12" md="6" offset-md="3" class="mt-3 text-center"> 
            <h4>Note: Here you can add another room type information.</h4>
        </v-col>
        <v-col sm="12" md="6" offset-md="3"> 
            <v-text-field
                color="orange"
                v-model="knkroomtype_information.name"
                label="Room Type"
                required
                outlined
                 :counter="50"
...
<script>
  ...
  export default {
    ...
    mounted(){
        this.$store.state.title = "Room Type";
    },
    computed: {
      showHeaders () {
    ...
    data: () => ({
      ...
      page: 1,
        ...
        valid: true,
        nameRules: [
        v => !!v || 'Room type name is required',
        ],
        confirmation: false,
    ...
    methods: {
        ...
        SaveKnkRoomtypeInformations(){
            let payload = {
                knkroomtype_information: JSON.stringify(this.knkroomtype_information)
                };
            knkroomtypeservices.saveKnkRoomtypeInformations(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.dialog = false;
                    this.FetchKnkRoomtypeInformation();
        ...
        ClearSaveDivision(){
            this.knkroomtype_information.name = '';
        },
        confirmUpdate(){
            let payload = {
                id: this.forAllUpdate['id'],
                knkroomtype_information: JSON.stringify({
                    name: this.knkroomtype_information.name
                })
            }
            knkroomtypeservices.updateKnkRoomtypeInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Room Type information has been updated.';
            this.$store.state.snackbar.show = true;
            this.FetchKnkRoomtypeInformation();
        ...
        UpdateKnkRoomtypeInformation(id, name){
            this.foUpdate = {
                id: id,
                name: name
            }
        ...
        confirmDelete(){
            ...
            knkroomtypeservices.removeKnkRoomtypeInformation(payload, this.$store.getters.getAccessToken).catch(error => console.log(error));
            this.$store.state.snackbar.message = 'Roomtype information has been removed.';
            this.$store.state.snackbar.show = true;
            this.confirmation = false
            this.$store.state.snackbar.colors = 'xgreen';
            this.$store.state.snackbar.icon = "mdi-check-circle-outline";   
        },
        RemoveKnkRoomtypeInformation(id, data){
            this.foDelete = {
                id: id,
                data: data
            }
          ...
          UpdateAllDivision(id, item){
            console.log(id, item)
            this.knkroomtype_information.name = item.name
            this.forAllUpdate = {
                id: id,
            }
