...
<template v-if="$vuetify.breakpoint.mdAndUp">
  ...
  <v-btn
    ...
    color="orange"
    elevation="2"
    @click="ExportCashFundInformation()"
    v-if="$store.getters.valAccess(261)"
  >
...
import cashfundreportservices from "../services/cashfundreportservices";
  import moment from 'moment';
     export default {
      ...
      data: () => ({
        page: 1,
        pageCount: 0,
        exportimport: [{ text: 'Download', icon: 'mdi-download-outline', linktrigger: '1' },
      ...
      mounted(){
          this.$store.state.title = "Cash Fund Report";
      },
      watch: {
            menu_year(val) {
      ...
      methods: {
        ...
        checkdaily(){
          ...
          getDates(){
            ...
            let payload = {
                      cashfund_report: JSON.stringify(filters),
                      business_branch_id: this.$store.getters.getBranchID
                  };
              cashfundreportservices.fetchCashFundReportInformation(payload, this.$store.getters.getAccessToken).then(response => {
                if(response.data.success){
                    this.cashfundreport = response.data.result;
                    this.cashfundreportlength = this.cashfundreport.length;
                    for (var z = 0 ; z < this.cashfundreportlength ; z++){
                    this.amount = parseFloat(this.cashfundreport[z].amount);
        ...
        // End Date range and fetch record
        // Export Cash fund
        ExportCashFundInformation(){
          var filters = {
                date_from: this.date_from,
                date_to: this.date_to
            }
          ...
          cashfundreportservices.exportCashFundInformation(payload, this.$store.getters.getAccessToken)
          .then(response => {
              const url = window.URL.createObjectURL(new Blob([response.data]))
              const link = document.createElement('a')
              link.href = url
              link.setAttribute('download','CashFund.xlsx')
